...
const CafeteriaTask3 = () => {
	...
	const createColumns = (columns, type, onclickFn) => {
		...
		for (var i=0;i<columns.length;i++) {
			...
			let oneColumn = <div key={columKey} className="col-xl-6 col-md-6 mb-6"><div className="column_wrapper" onClick={() => onclickFn(column.id)}><div className="icon_container"><CupHotFill className="icon"/></div><div className="column_txt">{column.price ?  column.amount + "x " + column.name + " " + column.price : column.txt}</div></div></div>
	...
	const buyDish = (id) => {
		...
		if (dishes[id].amount < 1) {
			let msg = "Sorry, no more left of " + dishes[id].name + " today.";
			setFeedback(msg);
			showNotification("Notification", msg, false, [{caption: "Okay", onClick: hideNotification}]);
		} else {
			dishes[id].amount--;
